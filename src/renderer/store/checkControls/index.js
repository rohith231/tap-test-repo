...
export default {
  ...
  actions: {
    ...
    GET_ALL({commit, rootState}, {payload}) {
      ...
      lastFramework = framework_id
      commit('user/SET_STATE', {fetchLoading: true}, {root: true});
      let url = `system/allframework?system_id=${system_id}&framework_id=${framework_id}&pageNumber=${pageNumber}&itemsPerPage=${itemsPerPage}&query_search=${query_search}&sortKey=${sortKey}&descSort=${descSort}&framework=${framework}`;
      
      if(framework=='NIST80053R5'){
        url = `system/framework?system_id=${system_id}&framework_id=${framework_id}&pageNumber=${pageNumber}&itemsPerPage=${itemsPerPage}&query_search=${query_search}&sortKey=${sortKey}&descSort=${descSort}&framework=${framework}`;
      }
      apiClient.get(`${apiPrefix}/${url}`).then(response => {
        lastFramework = null
        const {result, statusCode} = response.data
