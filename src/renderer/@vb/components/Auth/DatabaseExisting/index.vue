  ...
  <div class="card card-top card-top-primary" :class="$style.container">
    ...
    >
      ...
      <a-button
        ...
        >Connect</a-button
      >
      <a-button @click="resetForm" type="link" class="mr-3 mb-3 pull-right" :disabled="loading"
        >Reset</a-button
      >
    </a-form>
  </div>
...
export default {
  ...
  setup() {
    ...
    const formState = reactive({
      host: store.getters['database/DBHost'],
      username: store.getters['database/DBUsername'],
      password: store.getters['database/DBPassword'],
      database: store.getters['database/DBName'],
      port: store.getters['database/DBPort'],
    })
    ...
    const submit = () => {
      ...
      if (formState.host && formState.username && formState.password && formState.database) {
        store
          .dispatch('database/DATABASE_CONNECTION', { payload: formState })
          .then((returnState) => {
            if (returnState && hasConnection.value) {
              router.push('/system-sections/dashboard')
            }
          })
      }
    ...
    const testConnection = async () => {
      ...
      if (formState.host && formState.username && formState.password && formState.database) {
        store.dispatch('database/DATABASE_CONNECTION', { payload: formState })
      }
    }
    const tryDemo = () => {
      resetForm()
